GeoPortal.Widget.ListFeatureInfo = GeoPortal.Widget.extend({
    ...
    _createWidget: function(){
		...
		this._elemBlock = $('<a class="more-info__photo-block gallery-block" href="#"/>').appendTo(this._mainElement);
		this._elemBlock.append('<img src="img/region-page/ajax-loader.gif" alt="" class="ajax-loader"/>');
		
		this._firstEisElem = null;
		var files = this._feature.eisStore;
		if(files) {
        ...
        apiJsonGET(GeoPortal.basePath + "/layers/eis/"+this._layerId+"/"+this._feature.fid+"?random="+Math.random(),{},M.Util.bind(function(data){
            ...
            if(data && data.files && data.files.length) {
                ...
                for(i=0;i<len;i++){
                    ...
                    if(file.type.name == 'photo' && firstPhoto == undefined){
                        this._firstEisElem = {
                            type: 'photo',
                            file: file
                        };
